                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('admin.contests.index') }}">{{ __('admin/tour.index.title') }}</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/tour.edit.edit') }}<code>{{ $tour->title }}</code></li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    <a href="{{ route('admin.contests.index') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/tour.show.back') }}</a>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <form action="{{ route('admin.contests.update', $tour->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/tour.fields.title') }}" value="{{ $tour->translate($key)->title }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/tour.fields.slug') }}" value="{{ $tour->slug }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                            ...
                            <div class="card card-box custom mb-10" id="accordionWork">
                                ...
                                <div id="collapseWork" class="card-body show" data-parent="#accordionWork" >
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/tour.fields.content') }}" class="textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ $tour->translate($key)->content }}</textarea>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/tour.fields.num_of') }}</label>
                                        <div class="col-sm-12 col-md-12">
                                            <input name="num_of" placeholder="{{ __('admin/tour.fields.num_of') }}" value="{{ $tour->num_of }}" class="border-gray-300 rounded-md shadow-sm form-control @error('num_of') border border-danger @enderror" type="text"/>
                                            @error('num_of')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>
                                    </div>

                                    
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/tour.fields.label_text') }}</label>
                                <div class="col-sm-12 col-md-12">
                                    <ul class="nav nav-tabs" role="tablist">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <li class="nav-item @if($errors->has($key.'*label_text'))  border border-danger @endif">
                                                <a class="nav-link @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" data-toggle="tab" href="#label_text-{{ $key }}" role="tab">{{ $lang }}</a>
                                            </li>
                                        @endforeach
                                    </ul><!-- Tab panes -->
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="label_text-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[label_text]" placeholder="{{ __('admin/tour.fields.label_text') }}" value="{{ $tour->translate($key)->label_text }}" class="@if(LaravelLocalization::getCurrentLocale() == $key)  @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*label_text')) border border-danger @endif" type="text"/>
                                                @if($errors->has($key.'*label_text'))<span class="text-danger">{{ $errors->first($key.'*label_text') }}</span>@endif
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/tour.fields.num_of') }}</label>
                                        <div class="col-sm-12 col-md-12">
                                            <input name="label_color" placeholder="{{ __('admin/tour.fields.label_color') }}" value="{{ ($tour->label_color) ? $tour->label_color : '#7ab2fa' }}" class="colorpicker border-gray-300 rounded-md shadow-sm form-control @error('label_color') border border-danger @enderror" type="text"/>
                                            @error('label_color')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="contest_id" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('contest_id') border border-danger @enderror">
                                                <option value="" @if($tour->contest_type_id == '') selected @endif>{{ __('admin/video_comment.fields.choose') }}</option>
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" @if($tour->contest_type_id == $category->id) selected @endif>{{ $category->title }}</option>
                                                @endforeach
                                            </select>
