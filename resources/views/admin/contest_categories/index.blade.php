        ...
        <div class="page-header mb-0">
            <div class="row">
                ...
                <div class="col-md-6 col-sm-12 text-right">
                    @if(AdminCan('contestcategory-create'))
                        <a href="{{ route('admin.contest.categories.create') }}" class="btn btn-primary btn-sm scroll-click"><i class="fa fa-plus"></i> {{ __('admin/tour_category.index.create') }}</a>
                    @endif
                    @if(AdminCan('contestcategory-forcedelete'))
                        <a href="{{ route('admin.contest.categories.archive') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/tour_category.index.archive') }}</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="p-6 text-gray-900">
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <form name="search_form" method="post" action="{{route('admin.contest.categories.search')}}" >
                            @csrf
                ...
                <form name="destroy_all" method="post" action="{{route('admin.contest.categories.destroy_all')}}" >
                    @csrf
                    {{--@foreach ($cats as $sub)--}}
                    {{--{{ $sub->title }}--}}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="table-actions">
                                            @if(AdminCan('contestcategory-edit'))
                                                <a href="{{ route('admin.contest.categories.edit',$category->id) }}" data-color="#265ed7" style="color: rgb(38, 94, 215);"><i class="icon-copy dw dw-edit2"></i></a>
                                            @endif
                                            @if(AdminCan('contestcategory-delete'))
                                                ...
                                                <x-modal name="confirm-deletion_{{ $category->id }}" focusable>
                                                    <div class="p-6">
                                                        ...
                                                        <div class="mt-6 flex justify-end">
                                                            ...
                                                            <a href="{{ route('admin.contest.categories.delete',$category->id) }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-3">
                                                                {{ __('admin/common.messages.confirm_delete') }}
                                                            </a>
                                                        </div>
                                                    </div>
                                                </x-modal>
                                                {{--                                                <a href="{{ route('admin.contest.categories.delete',$category->id) }}" data-color="#e95959" style="color: rgb(233, 89, 89);"><i class="icon-copy dw dw-delete-3"></i></a>--}}
                                            @endif
